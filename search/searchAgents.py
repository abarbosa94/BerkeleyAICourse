...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState) :
        ...
        "*** YOUR CODE HERE ***"
        self.startSpace = (self.startingPosition, frozenset())
    def getStartState(self):
        """
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                if nextState in self.corners:
                    list_of_frozensets = [visited_corners,
                            frozenset([nextState])]
                    visited_corners = frozenset().union(*list_of_frozensets)
                nextState = (nextState,visited_corners)
                successors.append( ( nextState, action, 1) ) #The cost here will be always 1
...
def cornersHeuristic(state, problem):
    ...
    while nearestCorner:
        distance, corner= min([(util.manhattanDistance(positionA, corner), corner) \
                                for corner in nearestCorner])
        heuristic += distance
        positionA = corner
        nearestCorner.remove(corner)
...
def foodHeuristic(state, problem):
    ...
    for food in remainingFoods:
        closestFood.append((util.manhattanDistance(position,food), food))
    distance, food = min(closestFood)
